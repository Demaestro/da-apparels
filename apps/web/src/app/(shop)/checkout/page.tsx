        ...
        <div className="space-y-6">
              ...
              <div key={i} className="flex justify-between gap-4 text-sm">
                <div>
                  ...
                  <p className="font-sans text-xs text-obsidian-400">
                    {[item.variantLabel, item.fabricName].filter(Boolean).join(" · ")}
                    {" "}× {item.quantity}
                  </p>
                </div>
          ...
          {step === "review" && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
              <Button
                ...
                loading={false}
              >
                Pay {formatPrice(orderTotal + shippingFee, currency)}
              </Button>
          ...
          {step === "processing" && (
            <div className="text-center py-6 space-y-3">
              ...
              <p className="font-sans text-xs text-obsidian-400">Redirecting to payment…</p>
            </div>
          )}
        </div>
      </div>
